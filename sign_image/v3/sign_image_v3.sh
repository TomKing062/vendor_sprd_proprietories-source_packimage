...
doImgHeaderInsert()
{
    ...
    do
        if [ -f $loop ] ; then
            ./sprd_sign insert_image_header --image $loop --config_dir config/
            ret=$?
            if [ "$ret" = "1" ]; then
                 echo "####sprd_sign insert_image_header --image $loop failed!####"
                 return 1
            fi
        else
...
doSignImage()
{
    ...
    do
        if [ -f $image ]; then
            ...
            ./sprd_sign sign_image --image $image --config_dir config/ --algorithm rsa4096 --rsa_padding 6
        else
...
doImgHeaderInsert $SPL $EMMC_SPL $UFS_SPL $FDL1 $UBOOT $FDL2 $LK $LKFDL2 $SML $TEECFG $TOS
doSignImage $SPLSIGN $EMMC_SPLSIGN $UFS_SPLSIGN $FDL1SIGN $UBOOTSIGN $FDL2SIGN $LKSIGN $LKFDL2SIGN $SMLSIGN $TEECFGSIGN $TOSSIGN
