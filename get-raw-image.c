...
int main(int argc, char** argv)
{
    ...
    strncpy(newFilename, filename, len - 9);  // 复制原文件名，去掉原来的后缀
    strncpy(newFilename + len - 9, ".bin", 4);  // 加上新的后缀
    newFilename[len -5] = '\0';  // 添加字符串结束符
    mem += 0x200;
    FILE* file = fopen(newFilename, "wb");  // 以二进制模式打开文件
